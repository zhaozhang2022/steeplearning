...
p_init = 20.0 * 1e6
ptensor0=torch.ones(ncell,dtype=torch.float64)*p_init
swtensor0=torch.ones(ncell,dtype=torch.float64)*0.2
tol=1e-11
...
for t in range(nt):
    ...
    for epoch in range(num_epochs):
        ...
        if resnorm < tol:
            ptensor0 = ptensor1.clone().detach()
            swtensor0 = swtensor1.clone().detach()
            allpress[t]=ptensor0.cpu().numpy()
            allsw[t]=swtensor0.cpu().numpy()
            iflag=1
            break
        loss.backward()
        # jac = predict.grad.clone().reshape((1, ncell * 2))
        # jacinv = torch.pinverse(jac).reshape(ncell * 2)
        ...
        # for ie in range(ncell*2):
        #     resvec[ie].backward(retain_graph=True)
        #     jac[ie, :] = predict.grad.clone()
        #     predict.grad.zero_()
        # predict.data -= torch.matmul(torch.linalg.pinv(jac), resvec).data
    print('final total epochs are ', epoch, 'resnorm is: ', resnorm1, ' and ', resnorm2)
    print('total is ', resnorm)
    if iflag==0:
        print('newton doesnot converge at maxstep', t)
        sys.exit()
...
# np.savetxt('allpress-newtonlearn-v3d-ex1b.txt',allpress)
# np.savetxt('allsw-newtonlearn-v3d-ex1b.txt',allsw)
# print("output to vtk")
# f = open('result_NewtonLearn-dt200k-nt60-V3d-ex1b.vtk','w')
